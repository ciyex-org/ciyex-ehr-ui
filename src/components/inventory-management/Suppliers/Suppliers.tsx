...
export default function Suppliers() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (form.phone) {
      if (/^-/.test(form.phone.trim())) errs.phone = "Phone number cannot start with a negative sign";
      else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-lg rounded-xl border bg-white dark:bg-gray-900 dark:border-gray-700">
            ...
            <form onSubmit={save} className="p-6 grid grid-cols-2 gap-4 text-sm">
              ...
              <div><Label>Phone</Label><Input type="tel" value={form.phone} onChange={e => { const v = e.target.value.replace(/[^\d\s().+]/g, "").replace(/^-+/, ""); F("phone", v); if (formErrors.phone) setFormErrors(p => { const n = {...p}; delete n.phone; return n; }); }} maxLength={20} className={formErrors.phone ? "border-red-400" : ""} placeholder="e.g. (555) 123-4567" />{formErrors.phone && <p className="text-xs text-red-500 mt-1">{formErrors.phone}</p>}</div>
