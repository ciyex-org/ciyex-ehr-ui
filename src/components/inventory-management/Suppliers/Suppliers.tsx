...
export default function Suppliers() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (form.phone) {
      ...
      if (/[-]/.test(trimmed)) errs.phone = "Phone number cannot contain negative signs";
      else {
        const phoneDigits = trimmed.replace(/\D/g, "").length;
        ...
        else if (!/^\+?[\d\s().]{10,20}$/.test(trimmed)) errs.phone = "Please enter a valid phone number";
      }
    }
